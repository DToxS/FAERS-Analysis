...
##read data
kd = fread("reference_files/aan4368_Table_S3-kd.csv", sep = ",", header = T)
ki.smiles = fread("reference_files/KI-smiles.2.csv")
ki = read.csv("reference_files/KI-ecfp2.csv", sep = ",", header = F)
#kd = fread("data/41587_2011_BFnbt1990_MOESM5_ESM-davis.csv", sep = ",", header = T)
kd.names = names(kd)
coen.scores = fread("reference_files/KI-risk-score-training.csv", sep = ",", header = T)
###figure 4d 
library(ggrepel)
...
ki = read.csv("reference_files/KI-tanimoto.csv", sep = ",", header = T)
