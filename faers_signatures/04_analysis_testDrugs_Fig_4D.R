...
pdf("results/figure_external_prediction.pdf")
plot(pred_comb$zscore, pred_comb$predicted,pch=19,col="blue",xlim=c(0,1.7), ylim=c(0,1.7),
     xlab="Reporting odds ratio from FAERS",
     ylab="Predicting reporting odds ratio from signature",
     main="")
text(pred_comb$zscore+.1,pred_comb$predicted,labels = toupper(substr(pred_comb$drug,1,3)))
abline(0,1,lty=2)
dev.off()
